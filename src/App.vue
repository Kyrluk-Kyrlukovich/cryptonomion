      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            >
              <span
                  v-for="(coin, idx) in searchCoins"
                  :key="idx"
                  @click="clickAutoComplete(coin)"
                  class="
                  inline-flex
                  items-center
                  px-2
...
export default {
  ...
  created() {
    ...
    if (tickersData) {
      ...
      this.tickers.forEach(ticker => {
        subcribeToTickers(ticker.name, (newPrice, valid) => this.updateTicker(ticker.name, newPrice, valid))
      });
    }
  ...
  computed: {
    searchCoins() {
      return this.coinListAutoComplete
          .filter((elem) => elem.includes(this.ticker.toLocaleUpperCase()))
          .slice(0, 4);
    },
  ...
  methods: {
    ...
    updateTicker(tickerName, price, valid) {
      ...
      if(!valid) {
        filteredTickers.forEach(t => t.valid = false)
      } else {
        filteredTickers
          .forEach(t => {
    ...
    clickAutoComplete(nameCoin) {
      this.ticker = nameCoin;
      this.add();
    },
    ...
    add() {
      ...
      if (!this.tickers.find((elem) => elem.name == currentTicker.name.toLocaleUpperCase())) {
        this.tickers = [...this.tickers, currentTicker];
        this.filter = '';
        this.ticker = "";
        subcribeToTickers(currentTicker.name, (newPrice, valid) => {
    ...
    async fetchCoinList() {
      const f = await fetch(
          `https://min-api.cryptocompare.com/data/blockchain/list?api_key=154edae0836abebfb744393dc2ceeb33e6f53b9c0a36250bba6e9a4ef31ad48d`
      );
      const data = await f.json();
      const coinList = data.Data;
      this.coinListAutoComplete = Object.keys(coinList);
    },
